...
/**
 * Call Analytics Cache Service
 * Manages caching of call analytics data to reduce API calls
 */
class CallAnalyticsCacheService {
  constructor() {
    this.cacheDir = path.join(__dirname, '../../cache');
    this.cacheFile = path.join(this.cacheDir, 'call-analytics.json');
  ...
  async getCachedData(dateStr) {
    try {
      ...
      console.log(`💾 Using cached call data for ${dateStr}`);
      return cachedEntry.data;
      
    } catch (error) {
      console.error(`❌ Error getting cached call data for ${dateStr}:`, error.message);
      return null;
    }
  }
  ...
  async setCachedData(dateStr, data) {
    try {
      ...
      console.log(`💾 Cached call data for ${dateStr}`);
      
    } catch (error) {
      console.error(`❌ Error caching call data for ${dateStr}:`, error.message);
    }
  }
  ...
  async clearCachedData(dateStr) {
    try {
      ...
      cache.last_updated = new Date().toISOString();
      await this.saveCache(cache);
      console.log(`🗑️ Cleared call cache for ${dateStr}`);
    } catch (error) {
      console.error(`❌ Error clearing call cache for ${dateStr}:`, error.message);
    }
  }
  ...
  async clearAllCache() {
    try {
      ...
      console.log('🗑️ Cleared all call analytics cache');
      
    } catch (error) {
      console.error('❌ Error clearing all call cache:', error.message);
    }
  }
  ...
  async isTodayDataFresh(dateStr) {
    try {
      ...
      console.error(`❌ Error checking call data freshness for ${dateStr}:`, error.message);
      return false;
    }
  }
  ...
  async loadCache() {
    try {
      ...
      console.error('❌ Error loading call cache:', error.message);
      return {
        daily_data: {},
        last_updated: new Date().toISOString(),
        version: '1.0'
  ...
  async saveCache(cache) {
    try {
      ...
      console.error('❌ Error saving call cache:', error.message);
    }
  }
  ...
  async cleanupExpiredCache() {
    try {
      ...
      if (cleanedCount > 0) {
        cache.last_updated = new Date().toISOString();
        await this.saveCache(cache);
        console.log(`🧹 Cleaned up ${cleanedCount} expired call cache entries`);
      }
      
      return cleanedCount;
      
    } catch (error) {
      console.error('❌ Error cleaning up expired call cache:', error.message);
      return 0;
    }
  }
}

module.exports = CallAnalyticsCacheService;